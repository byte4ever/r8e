...
func main() {
	...
	for i := 1; i <= 8; i++ {
		result, err := rejectPolicy.Do(ctx, func(ctx context.Context) (string, error) {
			return fmt.Sprintf("request %d", i), nil
		})
		if errors.Is(err, r8e.ErrRateLimited) {
	...
	fmt.Println("Sending 7 requests (first ~5 instant, rest wait for tokens)...")
	start := time.Now()
	for i := 1; i <= 7; i++ {
		result, err := blockingPolicy.Do(ctx, func(ctx context.Context) (string, error) {
			return fmt.Sprintf("request %d", i), nil
		})
