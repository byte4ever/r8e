...
func main() {
	...
	// Successful call.
	result, err := paymentPolicy.Do(ctx, func(ctx context.Context) (string, error) {
	...
	attempt := 0
	result, err = notifyPolicy.Do(ctx, func(ctx context.Context) (string, error) {
		attempt++
		if attempt < 3 {
			return "", r8e.Transient(fmt.Errorf("notification service busy"))
		}
		return "notification sent", nil
	})
	fmt.Printf("  result: %q, err: %v (succeeded on attempt %d)\n", result, err, attempt)
	...
	result, err = barePolicy.Do(ctx, func(ctx context.Context) (string, error) {
		return "bare policy works", nil
	})
	fmt.Printf("  result: %q, err: %v\n", result, err)
}

// configDir returns the absolute path to a file in the same directory as this source file.
